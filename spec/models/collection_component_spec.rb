...
describe CollectionComponent do

	before :each do
		@series_component = CollectionComponent.new :series, "a test series component"
	end

	describe '#new' do
		it 'returns a new Component object' do
			@series_component.should be_an_instance_of CollectionComponent
		end
		it 'takes exactly 2 parameters' do
			lambda { CollectionComponent.new :series }.should raise_exception ArgumentError
			lambda { CollectionComponent.new :series, "a test series", :something_extra }.should raise_exception ArgumentError
		end
		it 'takes a Symbol as first argument, otherwise throw ArgumentError' do
			lambda { CollectionComponent.new 'series', 'this part is fine' }.should raise_exception ArgumentError
		end
		it 'only accepts a true component level type as first argument, otherwise throw ArgumentError' do
			lambda { CollectionComponent.new :sereez, 'this part is fine' }.should raise_exception ArgumentError
		end
	end
	...
	describe '#add_child' do
		it 'returns a new Component object' do
			(@series_component.add_child :subseries, "a test subseries child").should be_an_instance_of CollectionComponent
		end
		it 'takes exactly 2 parameters' do
		...
		it 'does not allow two children of the same name, raises IndexError' do
			(@series_component.add_child :series, "same name series").should be_an_instance_of CollectionComponent
	...
	describe '#children' do
		context "with children" do
			...
			it 'returns an Array of child Component\'s' do
				...
				@series_component.children[0].should be_an_instance_of CollectionComponent
			end
		end
	...
	describe '#parent' do
		context "with children" do
			...
			it 'returns an Array of child Component\'s' do
				@child_component.parent.should be_an_instance_of CollectionComponent
			end
		end
		context "with no children" do
			it 'returns an empty list (since no children)' do
	...
	describe '#find_child_by_designation' do
		...
		it 'returns the Component as specified by its designation.' do
			...
			x.should be_an_instance_of CollectionComponent
